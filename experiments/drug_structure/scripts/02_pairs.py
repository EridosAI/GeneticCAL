...
cross-boundary fraction (structurally dissimilar but co-lethal drugs).
"""
...
# Rank pairs by co-lethality (descending)
rank_order = np.argsort(-co_values)

# Determine feasible N levels
total_pairs = len(co_values)
N_LEVELS = []
for n in [25_000, 50_000, 100_000, 200_000]:
    if n <= total_pairs * 0.25:  # Don't take more than 25% of all pairs
        N_LEVELS.append(n)
    else:
...
# Convert triu indices to (i, j) pairs
pair_i = triu_idx[0]
pair_j = triu_idx[1]
...
for N in N_LEVELS:
    ...
    # Co-lethality scores for selected pairs
    co_scores = co_values[top_idx]

    # Cosine similarity for selected pairs
    ...
    # Sample N random negative pairs
    rng = np.random.default_rng(42)
    neg_idx = rng.choice(len(co_values), size=N, replace=False)
    # Ensure no overlap with positives
    ...
    # Cross-boundary AUC (only pairs with |cos| < 0.2)
    ...
    if cb_pos_mask.sum() > 50 and cb_neg_mask.sum() > 50:
        ...
        # For cross-boundary, cosine is near 0 for both → AUC should be ~0.5
...
# Log-scale histogram of co-lethality
...
# Cosine distribution of co-lethal pairs vs all pairs
for N in N_LEVELS[:3]:  # Top 3 levels
...
axes[1].set_xlabel("Morgan FP Cosine Similarity")
...
# Fig 2: Cosine vs co-lethality scatter (subsample for speed)
fig, ax = plt.subplots(figsize=(8, 6))
# Sample 500K pairs for scatter
...
ax.set_title(f"Structure vs Co-lethality ({n_sample:,} sampled pairs)")
...
if best_cb_frac < 0.02:
    ...
    print("  Same confounding as DepMap genes. Consider aborting.")
elif best_cb_frac >= 0.10:
    print(f"\n  ✓ EXCITING: Cross-boundary ≥ 10% — scaffold hopping regime!")
else:
