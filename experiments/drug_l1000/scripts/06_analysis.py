...
Poster children: high PAM score + low L1000 cosine + shared MOA/target.
"""
...
def load_best_model(embeddings):
    ...
    best_n = train_results[best_key].get("n_pairs",
                                          int(best_key.split("_")[-1]))

    for model_path in [MODELS / "pam_best.pt", MODELS / f"pam_{best_key}.pt",
        ...
        if model_path.exists():
            ...
            model.eval()
            return model, best_n
    raise FileNotFoundError(f"No model found for {best_key}")
...
if __name__ == "__main__":
    ...
    print("\n--- Poster child examples ---")
    print("  (High PAM score + low L1000 cosine + shared MOA/target)")
    ...
    for i, pc in enumerate(unannotated[:10]):
        bid_a = pc["broad_id_a"]
        bid_b = pc["broad_id_b"]
        match_a = drug_df[drug_df["broad_id"] == bid_a]
        match_b = drug_df[drug_df["broad_id"] == bid_b]
        moa_a = str(match_a.iloc[0].get("moa", "NA"))[:20] if len(match_a) > 0 else "?"
        moa_b = str(match_b.iloc[0].get("moa", "NA"))[:20] if len(match_b) > 0 else "?"
    ...
    axes[1].set_ylabel("Mean L1000 Cosine")
    ...
    # Fig 2: PAM vs cosine scatter with poster children
    ...
    ax.set_title("PAM vs Transcriptional Similarity")
    ...
    # Fig 3: PAM score by degree quintile
